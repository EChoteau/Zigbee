...
# /!\ Use of i_clk instead of clk /!\ # mAybe not...


#create_clock -name clk  -period $clk_period(clk) -waveform "0 [expr $clk_period(clk)/2]" [get_ports "clk"]
create_clock -name $clk_root(clk)  -period $clk_period(clk) -waveform "0 [expr $clk_period(clk)/2]" [get_pins "io_$clk_root(clk)/Y"]
