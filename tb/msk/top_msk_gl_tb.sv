...
module top_msk_tb();
    ...
    // -------------------------------------------------------------------------
    // Scénario de test (Optimisé pour la simulation Gate-Level)
    // -------------------------------------------------------------------------
    initial begin
        $display("--- DEBUT DE LA SIMULATION TOP MSK (Gate-Level / Post-Synthèse) ---");
        ...
        // On attend le prochain front DESCENDANT pour synchroniser les stimuli
        @(negedge s_clk);
        ...
        for (int i = 0; i < 10; i++) begin
            ...
            // On maintient le flag_enable pendant 1 cycle d'horloge complet
            @(negedge s_clk);
            s_flag_enable = 0;
            ...
            for (int ech = 0; ech < 25; ech++) begin
                ...
                repeat (3) @(negedge s_clk);
            end
        end

        // Laisser finir proprement la dernière forme (on peut utiliser posedge ici car on lit juste)
    ...
    // Celles-ci restent sur le posedge car elles surveillent les sorties du circuit !
    // -------------------------------------------------------------------------
