...
module tb_top_msk_bus;
    ...
    // ==========================================================================
    // CLOCK GENERATION (Fixé à 10 MHz -> Période 100ns)
    // ==========================================================================
    initial begin
        i_clk = 1'b0;
        forever #50 i_clk = ~i_clk; // #50 permet d'obtenir un cycle complet de 100ns
    end
    ...
    task automatic set_bus(logic [BUS_IN_WIDTH-1:0] bus_val);
    begin
        @(negedge i_clk);   // Injection propre au milieu du cycle d'horloge
        i_bus_in = bus_val;
        @(posedge i_clk);   // Attente du front montant de capture du DUT
    end
    endtask
    ...
    begin
        ...
        @(negedge i_clk);
        i_rst_n = 1'b1;
        repeat(2) @(posedge i_clk); // On laisse respirer l'ensemble de la Netlist
    end
    endtask

    // ==========================================================================
    // FONCTIONS D'ACCÈS AUX BUS SIGNÉS ET DÉBUG
    // ==========================================================================
    function automatic logic signed [MSK_RES-1:0] get_I();
        return o_bus_out[MSK_RES-1:0];
    endfunction
    ...
    function automatic logic get_dbg_b_enc();
        return o_bus_out[12];
    endfunction
    ...
    begin
        ...
        bus_val[2] = 1'b1; // b_in
        set_bus(bus_val);
        repeat (8) @(posedge i_clk); // Laisser la chaîne complète s'animer (Enc -> Demux -> Rom)

        #50; // On descend au milieu du cycle : les portes physiques de la Netlist sont figées
        signed_I = get_I();
        signed_Q = get_Q();
    ...
    begin
        ...
        #50; // On laisse le temps de propagation s'établir à travers le multiplexeur DFT
        dbg_b_enc_val = get_dbg_b_enc();

        assert (dbg_b_enc_val !== 1'bx)
            else $error("TOP FAIL: debug bus enc non initialise (contient X)");
        $display("TOP PASS: debug output presente, o_dbg_b_enc=%b", dbg_b_enc_val);
    end
    endtask
