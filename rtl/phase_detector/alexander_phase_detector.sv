`ifdef alexander_pd
    module phase_detector(clk,rst,sample,decision_in,decision_out, up,down);
        //inout definition
         ...
         bascule b1(.ck(sample),.rst(rst),.D(decision_in), .Q(a));
         bascule b2(.ck(~sample),.rst(rst),.D(decision_in), .Q(b));
         bascule b3(.ck(sample),.rst(rst),.D(a), .Q(c));
         bascule b4(.ck(sample),.rst(rst),.D(b), .Q(d));
